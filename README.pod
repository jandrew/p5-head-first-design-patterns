...
<a>
	<img src="https://img.shields.io/badge/this version-v0.0.1-brightgreen.svg" alt="this version">
</a>
...
line at the top to your library;
	...
	$use_lib //= '../../jandrew_lib/Chapter02';# s/jandrew_lib/${you}_lib/
...
Therefore all the patterns will be built as objects and stored in the 'jandrew_lib' folder 
under the chapter from the book.  The tests to run those patterns will be stored in the t 
folder under the chapter for the book as well.  If you want to run all the tests (except the 
GUI ones) at once run the file 03-tap_harness_tests.t.  If you fork the repo and delete/
rename the lib I provided then this approach should be sufficient to run your own learning 
cycle.  The goal is to treat your learning like Test Driven Development without having to 
write the tests.  (How great is that!)  If you want to test on the fly mid chapter you may 
need to comment out the use statements in each sub test for objects that you have not built 
yet.
...
(You can still use the current test.)  If you have a github user name it would probably be best 
to use that as your author string.  Example from the master test harness;
...
top level with prove.  I am far less likely to accept pull requests for code that is untested 
or fails to pass the books fairly minimal test suite.  If you do this I should get an 
automated report from Travis-CI that will check the work before I complete the pull request.

For now this readme is the kickoff place but I plan on putting all answers and general spewing 
of conciousness on the L<wiki|https://github.com/jandrew/p5-head-first-design-patterns/wiki>.  
For now there is no plan to provide complete pod for each class and role written.
...
with untested functionality.  If you have a question about design choices I will try my best 
but you may just get "It seemed right at the time!".  Please provide any concerns about the 
book content to the authors on L<their website
